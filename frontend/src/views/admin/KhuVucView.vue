    ...
    <CustomCard shadow="never">
      <div class="list-toolbar">
        ...
        <div class="list-actions">
          ...
          <CustomTooltip content="Khóa các khu vực đang sử dụng" placement="top">
            <span class="action-wrap">
          ...
          <CustomTooltip content="Mở các khu vực đang ngừng sử dụng" placement="top">
            <span class="action-wrap">
          ...
          <CustomTooltip content="Thêm khu vực" placement="top">
            <span class="action-wrap">
      ...
      >
        ...
        <CustomTableColumn prop="ma_khu_vuc" label="Mã khu vực" width="140" />
        <CustomTableColumn prop="ten_khu_vuc" label="Tên khu vực" min-width="200" />
...
import { API_KHU_VUC } from '@/constants/constant_api'
import {
  CustomButton,
  CustomCard,
  CustomCol,
...
const rules = {
  ma_khu_vuc: [
    { required: true, message: 'Vui lòng nhập mã khu vực', trigger: 'blur' },
...
function openEdit(row) {
  ...
  form.ma_khu_vuc = row.ma_khu_vuc
  form.ten_khu_vuc = row.ten_khu_vuc
...
async function submitForm() {
  ...
  const url = isEdit.value
    ? API_KHU_VUC.UPDATE(editingId.value)
    : API_KHU_VUC.CREATE
  const body = {
    ma_khu_vuc: form.ma_khu_vuc.trim(),
    ten_khu_vuc: form.ten_khu_vuc.trim(),
...
async function confirmRemove(row) {
  try {
    await ElMessageBox.confirm(
      `Xóa khu vực «${row.ten_khu_vuc}»? Thao tác không thể hoàn tác.`,
      'Xác nhận xóa',
  ...
  const url = API_KHU_VUC.DELETE(row.id)
...
async function confirmBulkRemove() {
  ...
  try {
    await ElMessageBox.confirm(
      `Xóa ${selectedCount.value} khu vực đã chọn? Thao tác không thể hoàn tác.`,
      'Xác nhận xóa',
      { type: 'warning', confirmButtonText: 'Xóa', cancelButtonText: 'Hủy' },
    )
  } catch {
...
async function confirmBulkStatus(trangThai) {
  ...
  try {
    await ElMessageBox.confirm(
      `${actionLabel.charAt(0).toUpperCase() + actionLabel.slice(1)} ${ids.length} khu vực (trạng thái: ${statusLabel})?`,
