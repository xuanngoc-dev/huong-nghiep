  ...
  <div class="admin-crud">
    ...
    <CustomCard shadow="never">
      ...
      >
        ...
        <CustomTableColumn prop="ten_nganh" label="Tên ngành" min-width="180" />
        <CustomTableColumn label="Nhóm ngành" min-width="240">
          <template #default="{ row }">
            <template v-if="row.nhom_nganhs?.length">
              <CustomTag
                v-for="nhom in row.nhom_nganhs"
                :key="nhom.id"
                size="small"
                class="nhom-tag"
              >
    ...
    >
      <CustomForm ref="formRef" :model="form" :rules="rules">
        <CustomRow :gutter="16">
          ...
          <CustomCol :span="24">
            <CustomFormItem label="Nhóm ngành" prop="nhom_nganh_ids">
              <CustomSelect
                v-model="form.nhom_nganh_ids"
                multiple
                filterable
                collapse-tags
...
function openEdit(row) {
  ...
  form.nhom_nganh_ids = [...(row.nhom_nganh_ids || [])]
  form.trang_thai = row.trang_thai
  dialogVisible.value = true
}

async function fetchNhomNganhOptions() {
  const res = await request({
  ...
  nhomNganhOptions.value = res.ok ? (res.data ?? []) : []
}

async function fetchList() {
  const q = filters.keyword.trim()
  const trangThai = filters.trang_thai || ''
  const nhomNganhId = filters.nhom_nganh_id || ''
  const params = {
    ...(q ? { q } : {}),
    ...(trangThai ? { trang_thai: trangThai } : {}),
    ...(nhomNganhId ? { nhom_nganh_id: nhomNganhId } : {}),
    start: pagination.start,
    limit: pagination.limit,
  }

  const res = await request({ url: API_NGANH_HOC.LIST, params })
...
async function submitForm() {
  ...
  const body = {
    ...
    nhom_nganh_ids: form.nhom_nganh_ids || [],
