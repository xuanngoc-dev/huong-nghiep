<template>
  <div class="admin-crud">
          ...
          <CustomCol>
            <CustomFormItem label="Từ khóa">
              <CustomInput
                ...
                placeholder="Tên hoặc mã tôn giáo..."
    ...
    <CustomCard shadow="never">
      <div class="list-toolbar">
        ...
        <div class="list-actions">
          ...
          <CustomTooltip content="Khóa các tôn giáo đang sử dụng" placement="top">
            <span class="action-wrap">
          ...
          <CustomTooltip content="Mở các tôn giáo đang ngừng sử dụng" placement="top">
            <span class="action-wrap">
          ...
          <CustomTooltip content="Thêm tôn giáo" placement="top">
            <span class="action-wrap">
      ...
      >
        ...
        <CustomTableColumn prop="ma_ton_giao" label="Mã tôn giáo" width="140" />
        <CustomTableColumn prop="ten_ton_giao" label="Tên tôn giáo" min-width="220" />
    ...
    <CustomDialog
      ...
      :title="isEdit ? 'Sửa tôn giáo' : 'Thêm tôn giáo'"
      :width="720"
    >
        ...
        <CustomRow :gutter="16">
          <CustomCol :xs="24" :sm="8" :md="8" :lg="8" :xl="8">
            <CustomFormItem label="Mã tôn giáo" prop="ma_ton_giao">
              <CustomInput v-model="form.ma_ton_giao" placeholder="Ví dụ: PG" maxlength="50" />
            </CustomFormItem>
          </CustomCol>
          <CustomCol :xs="24" :sm="8" :md="8" :lg="8" :xl="8">
            <CustomFormItem label="Tên tôn giáo" prop="ten_ton_giao">
              <CustomInput
                v-model="form.ten_ton_giao"
...
import { API_TON_GIAO } from '@/constants/constant_api'
import {
  CustomButton,
  CustomCard,
  CustomCol,
...
const rules = {
  ma_ton_giao: [
    { required: true, message: 'Vui lòng nhập mã tôn giáo', trigger: 'blur' },
    { max: 50, message: 'Tối đa 50 ký tự', trigger: 'blur' },
  ],
  ten_ton_giao: [
    { required: true, message: 'Vui lòng nhập tên tôn giáo', trigger: 'blur' },
...
async function toggleStatus(row, enabled) {
  ...
  const res = await request({
    url: API_TON_GIAO.UPDATE(row.id),
...
function openEdit(row) {
  ...
  form.ma_ton_giao = row.ma_ton_giao
  form.ten_ton_giao = row.ten_ton_giao
...
async function fetchList() {
  ...
  const res = await request({ url: API_TON_GIAO.LIST, params })
...
async function submitForm() {
  ...
  const url = isEdit.value ? API_TON_GIAO.UPDATE(editingId.value) : API_TON_GIAO.CREATE
  const body = {
    ma_ton_giao: form.ma_ton_giao.trim(),
...
async function confirmRemove(row) {
  try {
    await ElMessageBox.confirm(
      `Xóa tôn giáo «${row.ten_ton_giao}»? Thao tác không thể hoàn tác.`,
      'Xác nhận xóa',
  ...
  const res = await request({ url: API_TON_GIAO.DELETE(row.id) })
  if (!res.ok) return
...
async function confirmBulkRemove() {
  ...
  try {
    await ElMessageBox.confirm(
      `Xóa ${selectedCount.value} tôn giáo đã chọn? Thao tác không thể hoàn tác.`,
      'Xác nhận xóa',
      { type: 'warning', confirmButtonText: 'Xóa', cancelButtonText: 'Hủy' },
    )
  } catch {
...
async function confirmBulkStatus(trangThai) {
  ...
  try {
    await ElMessageBox.confirm(
      `${actionLabel.charAt(0).toUpperCase() + actionLabel.slice(1)} ${ids.length} tôn giáo (trạng thái: ${statusLabel})?`,
